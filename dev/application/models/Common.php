...
class Common extends CI_Model {
	...
	function user_title_count($user_id){
	...
	public function hide_title($user_id,$title_id){
		$this->db->where('user_id',$user_id);
		$this->db->where('title_id',$title_id);
		$this->db->from('titles_hide');
		$query=$this->db->get();
		if ($query->num_rows()<1) {
			$this->db->insert('titles_hide', array('user_id'=>$user_id,'title_id'=>$title_id));
			return $this->db->insert_id();
		}
		return false;
	}
	public function hide_user_feedbacks($user_id,$uid){
		$this->db->where('user_id',$user_id);
		$this->db->where('uid',$uid);
		$this->db->from('hide_users_feedbacks');
		$query=$this->db->get();
		if ($query->num_rows()<1) {
			$this->db->insert('hide_users_feedbacks', array('user_id'=>$user_id,'uid'=>$uid));
			return $this->db->insert_id();
		}
		return false;
	}
	public function get_hidden_users($user_id){
		$this->db->select('uid');
		$this->db->where('user_id',$user_id);
		$this->db->from('hide_users_feedbacks');
		$query=$this->db->get();
	...
	public function get_hidden_titles($user_id){
		$this->db->select('title_id');
		$this->db->where('user_id',$user_id);
		$this->db->from('titles_hide');
		$query=$this->db->get();
    ...
    function select_data_by_condition($tablename, $contition_array = array(), $data = '*', $sortby = '', $orderby = '', $limit = '', $offset = '', $join_str = array(), $group_by='',$where_not_in='') {
        $this->db->select($data);
		...
		if(!empty($where_not_in) && is_array($where_not_in)){
			foreach($where_not_in as $key=>$options){
				$this->db->where_not_in($key,$options);	
			}				
