					...
					<?php $i=0; foreach($survey['questions'] as $question): $i++; ?>
						<div class="question-item">
							...
							<div class="question-options">
								...
								<span class="question-option"><input required type="radio" name="answers[<?=$question['question_id']; ?>]" value="1"  <?php if($question['answer']==1) echo 'checked ';  ?> <?php if(!empty($question['answer'])) echo 'disabled ';  ?>><?=$question['option1']; ?></span>
								<?php endif; ?>
								<?php if(!empty($question['option2'])): ?>
								<span class="question-option"><input required type="radio" name="answers[<?=$question['question_id']; ?>]" value="2" <?php if($question['answer']==2)echo 'checked'; ?> <?php if(!empty($question['answer'])) echo 'disabled ';  ?>><?=$question['option2']; ?></span>
								<?php endif; ?>
								<?php if(!empty($question['option3'])): ?>
								<span class="question-option   "><input required type="radio" name="answers[<?=$question['question_id']; ?>]" value="3" <?php if($question['answer']==3)echo 'checked'; ?> <?php if(!empty($question['answer'])) echo 'disabled ';  ?>><?=$question['option3']; ?></span>
								<?php endif; ?>
								<?php if(!empty($question['option4'])): ?>
								<span class="question-option   "><input required type="radio" name="answers[<?=$question['question_id']; ?>]" value="4" <?php if($question['answer']==4)echo 'checked'; ?> <?php if(!empty($question['answer'])) echo 'disabled ';  ?>><?=$question['option4']; ?></span>
								<?php endif; ?>
							</div>
							<?php else: ?>
							<div class="question-answer" style="width:100%; clear:both; overflow:hidden;">
								...
								<div class="button-wrapper"><a href="#" class="add_more_field">Add More</a></div>	
							</div>
							<?php endif; ?>
						</div>
					<?php endforeach; ?>
