    ...
    <div class="profile-page">
        <div class="profile-image-block">
            <div class="container">
                ...
                <?php if (!empty($this->session->userdata['mec_user']['id']) && $user_data['id'] == $this->session->userdata['mec_user']['id']) { ?>
                <span class="edit-profile-btn"><i class="fa fa-pencil" aria-hidden="true"></i><?php echo $this->lang->line('edit_profile'); ?></span>
                <?php } ?>
                <div class="restrict_user" style="" data-isrestricted="<?= $is_restricted ?>">
                    <i class="fa fa-ban"></i> <?= ($user_data['is_restricted']) ?> <?= ($is_restricted == 1) ? '<span>Unblock</span>' : '<span>Block</span>' ?>   <?= ucwords($user_data['name']) ?>
                </div>
                <br/>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $.ajax({
            type:'POST',
            url: '<?php echo site_url('UserProfile/feedbacks'); ?>',
        ...
        $('#show-followings').click(function(e) {
            ...
            $.ajax({
                type:'POST',
                url: '<?php echo site_url('UserProfile/followings'); ?>',
                data: {user_id: user_id}
            }).done(function(data){
                element.html(data);
        ...
        $('#show-friends').click(function(e) {
            ...
            $.ajax({
                type:'POST',
                url: '<?php echo site_url('UserProfile/showFriendsInProfile'); ?>',
                data: {user_id: user_id}
            }).done(function(data){
                element.html(data);
                $('#show-feedbacks').removeClass("blue-btn").addClass("normal-btn");
    ...
    $('.restrict_user').click(function (e) {
        e.preventDefault();
        var session_id = <?= $this->session->userdata['mec_user']['id']; ?>;
        var user_id = <?= $user_data['id']; ?>;
        var restrict_user = $(this).attr('data-isrestricted');

        var span = $(this);
        if(restrict_user == 0){
            $.ajax({
                url : '<?= site_url('UserProfile/block_user') ?>',
                type : 'POST',
                dataType: 'JSON',
                ...
                if(data.status == 1){
                    ...
                    span.find('span').html('Unblock');
                    span.attr('data-isrestricted', '1');
                }else{
            ...
            $.ajax({
                url : '<?= site_url('UserProfile/unblock_user') ?>',
                type : 'POST',
                dataType: 'JSON',
                data: {session_id: session_id, user_id: user_id},
            }).done(function (data) {
                if(data.status == 1){
                    toastr.success(data.message, 'Success', {timeOut: 5000});
                    span.find('span').html('Block');
                    span.attr('data-isrestricted', '0');
                }else{
                    toastr.warning(data.error_message, 'Error', {timeOut: 5000});
                }
            })
