...
class Questionnaire extends MY_Controller {
	...
	public function chart_barchart($id){
        ...
        foreach($survey['questions'] as $question){
			...
			if($question['qtype']=='checkbox'){
				foreach($question['choices'] as $choice){
					$rows[]=array('label'=>$choice['option'],'results'=>$this->survey_model->get_survey_option_answer_count($id,$question,$choice['id']));
				}
			}elseif($question['qtype']=='mcq'){
				for($i=1; $i<9;$i++){
					$option='option'.$i;
					if(!empty($question[$option])){
						$rows[]=array('label'=>$question[$option],'results'=>$this->survey_model->get_survey_option_answer_count($id,$question,$i));
					}					
				}
			}elseif($question['qtype']=='single'){
				$rows[]=array('label'=>$question['question'],'results'=>$this->survey_model->get_survey_option_answer_count($id,$question,1));
			}
			$item=array('question_id'=>$question['question_id'],'question'=>$question['question'],'qtype'=>$question['qtype']);
			$item['rows']=$rows;
			$questions[]=$item;
        } 
	...
	public function barchart($id){
		...
		$chart_data_path=base_url().'/admin/questionnaire/chart_barchart/'.$id;	
		$this->data['chart_data_path']=$chart_data_path;		
		//$this->load->view('admin/survey/barchart',$this->data);
		$this->data['title'] = $survey['title'].' - Questionnaire Results';		
		$this->template->admin_render('admin/survey/barchart', $this->data);
	}
